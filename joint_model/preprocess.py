...
def preprocess_semeval_dataset(path, is_test_set=False):
...
def preprocess_multirc_dataset(path, is_test_set=False):
    writer = open(path.replace('.json', '') + '-processed.json', 'w', encoding='utf-8')
    ex_cnt = 0
    p_num = 0
    for obj in json.load(open(path, 'r', encoding='utf-8'))['data']: 
        r_text = ' '.join([x[4:-4] for x in re.findall('</b>.*?<br>', obj['paragraph']['text'])])
        d_dict = tokenize(r_text)
        d_id = str(p_num)          
        qs = obj['paragraph']['questions']
        for q in qs:
            q_id = str(q['idx'])
...
def preprocess_mctest_dataset(path, is_test_set=False):
    writer = open(path.replace('.json', '') + '-processed.json', 'w', encoding='utf-8')
    ex_cnt = 0
    for p_num, para in enumerate(json.load(open(path, 'r', encoding='utf-8'))['data']):
        r_text = ' '.join(para[0])
        d_dict = tokenize(r_text)
        d_id = str(p_num)
        for q_num, qp in enumerate(para[1]):
            q_id = str(q_num)
            q_dict = tokenize(qp[0])
            for a_num, ans in enumerate(qp[1]):
                c_id = str(a_num)
                c_dict = tokenize(ans[0])
                if ans[1] == 'True':
                    label = 1
                elif ans[1] == 'False':
                    label = 0
...
if __name__ == '__main__':
    if len(sys.argv) > 1 and sys.argv[1] == 'conceptnet':
        preprocess_conceptnet('~/scratch/data/conceptnet/conceptnet.csv')
        exit(0)
    init_tokenizer()
    #preprocess_semeval_dataset('./data/trial-data.json')
    #preprocess_semeval_dataset('./data/dev-data.json')
    #preprocess_semeval_dataset('./data/train-data.json')
    #preprocess_semeval_dataset('./data/test-data.json')
    #preprocess_multirc_dataset('./data/train_456-fixedIds.json')
    #preprocess_multirc_dataset('./data/dev_83-fixedIds.json')
    preprocess_mctest_dataset('./data/train-500.json')
    preprocess_mctest_dataset('./data/dev-500.json')
    preprocess_mctest_dataset('./data/test-500.json')
    preprocess_mctest_dataset('./data/train-160.json')
    preprocess_mctest_dataset('./data/dev-160.json')
    preprocess_mctest_dataset('./data/test-160.json')
