...
def load_data(qa_path, nli_path):
    from doc import Example
    label_dict = {'entailment': 0, 'neutral': 1, 'contradiction': 2}
    choice_data = {}
    nli_data = []
    for line in open(qa_path, 'r', encoding='utf-8'):
        if (qa_path.find('race') < 0 or np.random.random() < 0.6) and line.strip() != '':
            ex = Example(json.loads(line))
            if ex.valid == 1:
                ex_id = '_'.join(ex.id.split('_')[-3:])
                choice_data[ex_id] = ex

    #print(choice_data.keys())
    for line in open(nli_path, 'r'):
        cols = line.split('\t')
        nli_data.append((cols[0].strip(), cols[1].strip(), label_dict[cols[2].strip()]))
    print('Load %d examples from %s...' % (len(choice_data), qa_path))
    return (choice_data, nli_data)
...
if __name__ == '__main__':
    # build_vocab()
    train_data, train_nli_data = load_data('./data/train-data-processed.json', './data/train-data-nli.txt')
    dev_data, dev_nli_data = load_data('./data/dev-data-processed.json', './data/dev-data-nli.txt')
    test_data, test_nli_data = load_data('./data/test-data-processed.json', './data/test-data-nli.txt')
    train_data_list = [train_data[key] for key in train_data]
    dev_data_list = [dev_data[key] for key in dev_data]
    test_data_list = [test_data[key] for key in test_data]
    build_vocab(train_data_list + dev_data_list + test_data_list)
