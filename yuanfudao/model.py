...
class Model:
    ...
    def evaluate(self, dev_data, debug=False, eval_train=False):
        ...
        if eval_train:
            ...
            with open('./results/' + self.dataset  + '_res', 'w') as outfile:
                for i, ex in enumerate(dev_data):
                    outfile.write('_'.join(ex.id.split('_')[-3:]) + '\t' + str(float(pred_scores[i].data)) + '\n')
                print(i)
            with open('./results/' + self.dataset  + '_gold', 'w') as outfile:
                for i, ex in enumerate(dev_data):
                    outfile.write('_'.join(ex.id.split('_')[-3:]) + '\t' + str(gold[i]) + '\n')
                print(i)
    ...
    def get_embeddings(self, example_data, out_path):
        eg_batch = self._iter_data(example_data)
        e_writer = open(out_path, 'w')        
        for eg_inp_ in eg_batch:
            eg_inp = [x for x in eg_inp_[:-1]]
            embs = self.network.get_embeddings(*eg_inp)
            p_embs, q_embs, c_embs = embs
            for p_emb in p_embs:
                e_writer.write('p_emb\t' + str(list(p_emb)) + '\n') 
            for q_emb in q_embs:   
                e_writer.write('q_emb\t' + str(list(q_emb)) + '\n')
            for c_emb in c_embs:
                e_writer.write('c_emb\t' + str(list(c_emb)) + '\n')
        e_writer.close()
