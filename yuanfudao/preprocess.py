...
def preprocess_race_dataset(d):
    ...
    writer = open('./data/race/race-processed.json', 'w', encoding='utf-8')
    ex_cnt = 0
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        print('Specify the name of the dataset to preprocess (conceptnet/semeval/multirc).')
        exit(0)
    elif len(sys.argv) > 1:
        for arg in sys.argv[1:]:
            if arg not in ['conceptnet', 'multirc', 'semeval']:
                print('Identifier "', arg, '" is not recognized')

    if 'conceptnet' in sys.argv == 'conceptnet':
        preprocess_conceptnet('~/scratch/data/conceptnet/conceptnet.csv')

    init_tokenizer()
    if 'semeval' in sys.argv:
        preprocess_semeval_dataset('./data/semeval/dev-data.json')
        preprocess_semeval_dataset('./data/semeval/train-data.json')
        preprocess_semeval_dataset('./data/semeval/test-data.json')
    if 'multirc' in sys.argv:
        preprocess_multirc_dataset('./data/multirc/train_456-fixedIds.json')
        preprocess_multirc_dataset('./data/multirc/dev_83-fixedIds.json')
    if 'race' in sys.argv:
