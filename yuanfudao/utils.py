...
def load_data(path):
    ...
    for line in open(path, 'r', encoding='utf-8'):
        if (path.find('race') < 0 or np.random.random() < 0.6) and line.strip() != '':
...
if __name__ == '__main__':
    ...
    dev_data = load_data('./data/semeval/dev-data-processed.json')
    test_data = load_data('./data/semeval/test-data-processed.json')
    build_vocab(train_data + dev_data + test_data)
